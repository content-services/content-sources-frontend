...
const Checkbox = ({ rowData, rowIndex }: CheckboxProps) => {
  ...
  const isCustomEPEL = origin === ContentOrigin.EXTERNAL && isEPELUrl(rowData.url);
  const shouldDisableCustomEPEL = isCustomEPEL && !isInOtherUUIDs(rowData.uuid);
  ...
  return (
    <TdWithTooltip
      ...
      select={{
        ...
        isDisabled: noSnapshotAvailable || shouldDisableOtherEPEL || shouldDisableCustomEPEL,
      }}
    />
  );
};
