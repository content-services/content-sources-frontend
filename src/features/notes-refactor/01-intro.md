...
Please go through these notes before reviewing the code. Some of the choices might seem too complicated and not so common in React apps. You may dislike them at first. However, there are sound reasons behind them that I want to convey to you via this document.
...
The refactor proposes a different structure of code from the current one. The concepts it draws from can be labeled as "vertical sliced" or "feature sliced" with a "domain core modelled with functions" and "ports and adapters" at the edges. I find it is all a variation on the old question of modularity.
...
I believe the app outgrew its current code structure, as it has evolved into a bigger codebase with multiple functionalities. These functionalities are not explicit and are obfuscated by the current structure. The need for a different code organization came up from the problems I had experienced, which I describe in detail below. And since the frontend is not yet so big, I wanted to try to apply this structure before it would get too big.
