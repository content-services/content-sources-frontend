...
import { hasPackageUUID, PackageItem, PackageItemWithUUID } from 'services/Content/ContentApi';
...
import { OnSetPage, Pagination } from '@patternfly/react-core';
import spacing from '@patternfly/react-styles/css/utilities/Spacing/spacing';
import { PackageTableFilters } from './usePackageTableFilters';
...
type FilterProps = ReturnType<typeof useDataViewFilters<PackageTableFilters>> & {
  debouncedSearch: string;
};

type PackageVariant = PackageItem | PackageItemWithUUID;

interface PackagesTableProps<T extends PackageVariant> {
  isFetching: boolean;
  isLoading: boolean;
  count: number;
  packagesList: T[];
...
export const usePackagesTableSelection = () => {
  const selection = useDataViewSelection({ matchOption: (a, b) => a.id === b.id });
  return selection;
};

const PackagesTableWithToolbars = <T extends PackageVariant>({
  packagesList,
  paginationProps,
