...
const ContentListContextProvider = ({
  children
}: ContentListContextProviderProps) => {
  ...
  const [filterData, setFilterData] = useState<filterData>({ searchQuery: '', versions: [], arches: [] })

  const filters = {
    searchQuery, selectedVersions, selectedArches
  }
  ...
  const getLabels = (type: string, names: Array<string>) => {
    const namesLabels = type === 'arch' ? data['distribution_arches'] : data['distribution_versions'];
