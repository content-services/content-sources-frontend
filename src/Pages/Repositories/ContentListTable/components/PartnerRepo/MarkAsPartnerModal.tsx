...
import { useToggleAsPartnerMutate } from 'services/PartnerRepos/PartnerReposQueries';
...
export default function MarkAsPartnerModal() {
  ...
  const { mutateAsync, isPending } = useToggleAsPartnerMutate();

  const onConfirm = async () => {
    if (!repoUUID) return;
    // Only marking repository as partner is allowed, not unmarking
    await mutateAsync({ uuid: repoUUID, partner: true });
    onClose();
  };
      ...
      <ModalFooter>
        <Flex columnGap={{ default: 'columnGapLg' }}>
          <Button
            ...
            variant='primary'
            isLoading={isPending}
            isDisabled={!understood || isPending || !repoUUID}
            onClick={onConfirm}
          >
