...
test.describe('Templates CRUD', () => {
  test('Add, Read, update, delete a template', async ({ page }) => {
    ...
    await test.step('Create a repository', async () => {
      await bulkCreateRepos(page, 1, repoNamePrefix);
    });
    ...
    await test.step('Create a template', async () => {
      ...
      const modalPage = page.getByTestId('add_template_modal').page();
      await modalPage.getByRole('textbox', { name: 'Filter by name' }).fill(smallRHRepo);
      await modalPage
        .getByRole('row', { name: smallRHRepo })
    ...
    await test.step('Read and update values in the template', async () => {
      ...
      await expect(page.getByText('Define template content')).toBeVisible;
      await page.getByRole('button', { name: 'Next', exact: true }).click();
      await expect(page.getByText('Additional Red Hat repositories')).toBeVisible;
      await page.getByRole('button', { name: 'Next', exact: true }).click();
      await expect(page.getByText('Custom repositories')).toBeVisible;
      await expect(page.getByText(`${repoName}`)).toBeVisible();
      await page.getByRole('button', { name: 'Next', exact: true }).click();
      await expect(page.getByText('Set up date')).toBeVisible;
      await page.getByRole('button', { name: 'Next', exact: true }).click();
