  ...
  test('should warn against template deletion when associated to a system and not warn after unregistration', async ({
    ...
    await test.step('Attempt to delete template and verify warning appears', async () => {
      ...
      await test.step('Verify deletion warning appears for template with associated systems', async () => {
        ...
        // Wait for the Remove button to be enabled after status check
        const removeButton = modal.getByRole('button', { name: 'Remove' });
        await expect(removeButton).toBeEnabled();

        // Check for the specific warning text found in the modal
        ...
        // Assert that the warning text is present
        expect(hasWarningText).toBeTruthy();

        // Cancel the deletion dialog since warning appeared as expected
    ...
    await test.step('Verify template can now be deleted without warning', async () => {
      ...
      await test.step('Verify no warning appears and deletion succeeds', async () => {
        ...
        // Verify no warning text is present
        const hasWarningText =
          /This template is in use|template is assigned to|Removing this template will cause/i.test(
            modalText,
          );
        ...
        // Verify standard confirmation text is present
