  ...
  test('Verify RHEL repos have a snapshot task queued within the last 60 minutes', async ({
    ...
    await test.step('Get RHEL repositories', async () => {
      ...
      for (const repo of rhelRepos!) {
        ...
        console.log(
          `Repo: ${repo.name}, Current time: ${new Date(now).toUTCString()} (${new Date(now).toUTCString()}), Task queued at: ${taskQueuedAt.toISOString()} (${taskQueuedAt.toUTCString()}), Age: ${Math.round((now - taskQueuedAt.getTime()) / 60000)} minutes`,
        );
