 ...
 * 2. READONLY user cannot create, update, or delete repos/templates (but can read)
 * 3. User from different org (stable_sam) cannot read repos and templates from admin's org
 */
...
test.describe('API RBAC Tests', () => {
  ...
  test.describe('READONLY user cannot modify resources', () => {
    test('Read-only user can read but cannot modify repositories', async ({
      ...
      await api.deleteRepository({ uuid: repoUuid }, withAuth(adminToken));
    });
    ...
    test('Read-only user can read but cannot modify templates', async ({ client, cleanup }) => {
      ...
      await api.deleteTemplate({ uuid: templateUuid }, withAuth(adminToken));
    });
  });
  ...
  test.describe('Cross-org isolation (stable_sam user from different org)', () => {
    test('User from different org cannot read repositories', async ({
      ...
      await test.step('stable_sam cannot list repositories from other org', async () => {
        const response = await api.listRepositories(
          { search: repoName, origin: 'external' },
    ...
    test('User from different org cannot read templates', async ({ client, cleanup }) => {
      ...
      await test.step('stable_sam cannot list templates from other org', async () => {
