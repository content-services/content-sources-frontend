...
test.describe('No Cross Organization Access', () => {
  ...
  test('Using a RHEL9 container, obtain identity certificate and test cross-org repository access', async () => {
    ...
    await test.step('Test cross-org repository access using identity certificate', async () => {
      ...
      if (certResult?.exitCode === 0 && keyResult?.exitCode === 0) {
        ...
        } else if (curlResult?.exitCode === 60) {
          console.log('CURL failed with exit code 60. SSL certificate verification failure');
          expect(curlResult.exitCode).toBe(60);
        } else if (curlResult?.exitCode === 6) {
          console.log('CURL failed with exit code 6. Could not resolve host (network issue)');
          expect(curlResult.exitCode).toBe(6);
        } else {
          console.log(`CURL failed with exit code ${curlResult?.exitCode}`);
  ...
  test('should search for Pulp repository and test snapshot access with NO_SUBS_USER', async ({
    client,
  }) => {
    test.skip(
